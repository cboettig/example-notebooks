...
A demonstration leaflet map showing polygons from [Global Mangrove Watch](globalmangrovewatch.org) (GMW) 2020 data overlaid with the 2020 values and change since 2017 in Impact Observatory's [Biodiversity Intactness Index](https://planetarycomputer.microsoft.com/dataset/io-biodiversity).  

Zoom in on coastal areas to see individual polygons.  Use the layer toggle to turn layers off.  The OpenStreetMap basemap is helpful as it documents protected areas (even adds mangrove icons), which do not always quite match those of GMW.  
...
map <- 
  ...
  tm_shape(bii_diff) + tm_raster(alpha = 0.75) +
